...
import org.apache.sling.tenant.spi.TenantSetup;
...
@Reference(name = "tenantSetup", referenceInterface = TenantSetup.class,
...
public class TenantProviderImpl implements TenantProvider {
    ...
    private SortedMap<Comparable<Object>, TenantSetup> registeredTenantHandlers =
        new TreeMap<Comparable<Object>, TenantSetup>(Collections.reverseOrder());
    ...
    private synchronized void bindTenantSetup(TenantSetup action,Map<String,Object> config){
        registeredTenantHandlers.put(ServiceUtil.getComparableForServiceRanking(config), action);
    }

    private synchronized void unbindTenantSetup(TenantSetup action,Map<String,Object> config){
        registeredTenantHandlers.remove(ServiceUtil.getComparableForServiceRanking(config));
    }
    
    private synchronized Collection<TenantSetup> getTenantHandlers() {
        ...
        if (adminResolver != null) {
            try {
                ...
                if (child != null) {
                    ...
                    for (TenantSetup ts: getTenantHandlers()) {
